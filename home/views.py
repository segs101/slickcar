...
def profile_update(request):
    ...
    if request.method == 'POST':
       ...
       if form.is_valid():
        ...
        messages.success(request, f'Dear {new} your profile has being updated')
...
def password_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Dear {new} your password has been updated successfully')
            return redirect('profile')
        else:
            messages.error(request, f'dear {new} a problem occurred with the following errors: {form.errors} ')
...
def pay(request):
    if request.method == "POST":
        api_key = 'sk_test_6dfb2403c7809252c36cf6f3f4978690926c152e' #secreat key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://34.203.212.21/callback' #payment confirmation page
